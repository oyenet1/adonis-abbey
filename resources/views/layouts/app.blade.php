	...
	<header class="navbar pcoded-header navbar-expand-lg navbar-light header-blue poppins-medium">
		<div class="m-header bg-green-600">
			...
			<a href="#!" class="b-brand">
				...
				<img src="/img/logo.png" alt="" class="logo w-12">
				...
				<li>
					<div class="dropdown drp-user text-white">
						...
						<div class="dropdown-menu dropdown-menu-right profile-notification">
							<div class="pro-head">
								@if (Auth::user()->image)
									<img class="h-10 w-14 rounded-full bg-red-600 object-cover object-center"
										src="{{ asset('storage/' . Auth::user()->image) }}" alt="{{ Auth::user()->name }}">
								@else
									<span
										class="mx-auto block max-w-min rounded-full p-2 text-2xl font-semibold uppercase shadow-inner">{{ Auth::user()->name[0] }}</span>
								@endif
								...
								<form class="logout-form hidden" id="logout-form2" action="{{ route('logout') }}" method="POST">
									@csrf
								</form>
							</div>
							<ul class="pro-body">
